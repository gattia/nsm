# NSM code-health refactor — executed history

**Repo:** `gattia/nsm`. **Split out of `NSM_CODE_HEALTH_REFACTOR.md` on 2026-09-21**, when
the active plan had reached 5,152 lines and could no longer be read to find out what to do
next. Everything here is **executed work**, verbatim and unedited.

> **You almost certainly do not need to read this file.** The live plan is
> `NSM_CODE_HEALTH_REFACTOR.md`; it carries the State block, the remaining slices and the
> open decisions. Come here for one reason: to answer "what did slice X actually do, and
> what did it believe beforehand". That is the `Diverged` value `CLAUDE.md` says exists
> nowhere else, which is why none of it is compressed.

## What is in here

| | |
|---|---|
| **The State narrative, 2026-08-14 → 2026-09-03** | ~1,450 lines of accumulated findings, written before the State block was cut back to the six fields `CLAUDE.md` prescribes. Superseded as a status report; kept as the record of what was found when. |
| **§1 Why** | The LR-schedule bug that triggered the refactor, and the measured state of the codebase at 2026-08-14. |
| **§3–§7** | Phases 0 through 3, all delivered: scope rulings, the codebase map, the documentation pass, the regression harness, and the two post-v0.3.0 validation runs. |
| **§8.0 slice index** | The scheduling table for all of A–T, including the three rows still open. **The P, S and T rows are restated, re-measured, in the live plan** — read them there, not here. |
| **§8.0.A – §8.0.R** | Eighteen executed slice statements, one per slice, each written as commit 1 of its own slice. This is the bulk of the file. |

Each slice statement's own PR carries the same material in reviewable form, and
`CHANGELOG.md`, `docs/KNOWN_ISSUES.md` and `docs/SCOPE.md` carry every durable fact that
came out of one. Nothing below is the only copy of anything a user needs.
...
- **Next:** **§8.0.P**, then **S**, then **T**. Q left the plan (see below). **Both
  ...
  to it**, verified in the production env. **Blocked on nothing.**
- **Merging is not deploying; the pull is the deploy (2026-09-02).** §10.1's consequence,
  written down because it is one step in a sequence nobody states: `kneepipeline` imports
  this working tree, so `main` moving on GitHub changes production not at all until
  someone runs `git pull` *here*. Pull with the queue idle — `steps.run_nsm` spawns a
  fresh subprocess per fit, so a mid-job pull splits one job across two library versions —
  and idle is checkable rather than assumed: the celery worker has no step children and
  holds only its bare CUDA context (~100 MiB) on the GPU. Then verify in the **production**
  env, not `nsm-dev`, which is §7.5a's lesson in one line. No worker restart is needed for
  a tree checkout; the case that does need one is a *conda package* upgrade, because the
  consumer's provenance stamp caches library versions per process.
...
- **0b's survey is answered, and measured rather than transcribed (2026-08-30).** The
  ...
  `interpolate_points` / `interpolate_mesh`, `Decoder`, `TriplanarDecoder`,
  `reconstruct_mesh`), zero references to `NSM.train`, `train_deep_sdf` or
  `sample_difficulty` in package, notebooks, scripts or tests, and an editable pin at
  `nsm@b7cfd49` in its lock file. **§8.0.P is ungated**; the ruling left at P is #18's
  port-or-delete, a feature call, no longer a consumer question. `SCOPE.md` §5 updated in
  the same commit — its guess about which rulings a mesh consumer would reach into was
  half right (`interpolate` yes, at both entry points; `refine_mesh` no).
- **Q is out of the plan (maintainer, 2026-08-30): sigma is its own initiative.** §8.3's
  test kept #3 in §8 — the same number means two things and one is wrong — and the
  maintainer moved it out anyway: passing the test says it is a fix, not that this plan
  must carry it, and a behaviour change with its own migration guard and release needs
  should not hold the refactor's close hostage, nor wait on it. It re-homes to its two
  standing plans (`BREAKING_CHANGE_PROPOSAL.md` the what/why,
  `SIGMA_COORDINATE_IMPLEMENTATION_PLAN.md` the how), whose State blocks now own it; the
  Q row, §8.3, §9 and §11 carry pointers. The sequencing constraint survives the move —
  a release on either side — and v0.4.0 (S) is the near one.
...
- [x] **0b — survey downstream consumers before quarantining anything.** *Surveyed
      2026-08-30, from a fresh `gattia/nsosim` clone, after the maintainer stated it is
      inference-only. Measured: its whole NSM surface is five symbols in `mesh`, `models`
      and `reconstruct`, and nothing anywhere references `NSM.train` or
      `sample_difficulty` — the sweep is recorded in `docs/SCOPE.md` §5. The physical
      quarantine move this gated is §8.0.P, now ungated.*
