...
### 2.1 The mesh cluster

`{refine_mesh, correspondence_metrics, triangle_metrics, interpolate}` is 2,104 lines,
16% of the library. Until v0.4.0, `NSM/mesh/__init__.py` imported only `main`, so these
four were reachable only by importing each submodule by name. They are now imported in
`NSM/mesh/__init__.py` and listed in `__all__`. They are not dead code (`SCOPE.md` §2.3).
...
| `reconstruct/main.py` | 5 | prod — `reconstruct_mesh` only since §8.0.E (2026-08-24: `get_mean_errors` moved, the dead pair deleted); re-import surface for all three split-off modules. §8.0.J (2026-08-27) split its body into `_build_reference_mesh` / `_sample_subject` / `_assemble_result`, all keyword-only; the 58-parameter public signature is untouched and belongs to §8.0.O with `mesh/main.py`'s two |
| `reconstruct/latent_fit.py` | 0 | prod — the latent-optimization stack, received from `main.py` (§8.0.C). §8.0.K (2026-08-27) split `reconstruct_latent`'s 191-line nested loss into `_select_samples` / `_recon_loss` / `_regularization_losses`, all keyword-only, and added `n_samples_per_chunk` (#75); the 38-parameter public signature is untouched and belongs to §8.0.O with `reconstruct_mesh`'s and `mesh/main.py`'s two |
...
| **Parameter accepted and silently ignored** | ~10 | `get_pts_center_and_scale`: `center=` / `scale=` are rebound before they are read, so both operations happen unconditionally. `n_pts_random` swallowed by `**kwargs` — the consumer passes 100,000 for it. *`models/` is swept ([#20](https://github.com/gattia/nsm/issues/20), §8.0.H): `normalize_coordinates`' `padding`, `Decoder`'s `xyz_in_all` and `latent_noise_sigma`, and `VAEDecoder`'s `activation` are deleted. `reconstruct_mesh`'s `**kwargs` refuses everything but the deprecated key since §8.0.J — measured, five of five misspellings of real parameters ran silently before it; `KNOWN_ISSUES.md` § History 20.* **Deleting the name is only half the fix where the function keeps `**kwargs`** — `Decoder` does, so each deleted argument raises when set truthy rather than going back to being ignored. *§8.0.L found the variant with no `**kwargs` to hide behind: a value read where it is named and acted on somewhere else. `train_epoch`'s `multi_object_overlap` raised 174 lines below the read, its `eikonal_weight` was accepted at the one entry point of three that did not gate it, and its `surface_weighting` length was a bare `assert` in the innermost loop — under `python -O` that one silently rescaled the loss. All three are refused at the top of the function now.* *§8.0.M found the variant where the shadow is a **config key of the same name**: `save_model_params`' `list_mesh_paths` argument was placed and then merged over by `dict_save.update(config)`, and the shipped `default_config.json` carries that key as `null`, so every default run recorded no subject list at all. It is also the case where the class's usual remedy inverts — the parameter is the authoritative source and the config's copy is the stale artifact, so the fix is precedence, not deletion. `KNOWN_ISSUES.md` § History 26.* *§8.0.N′ found the variant where the argument is honoured at one end of the function and hardcoded at the other: `compare_cart_thickness`'s `regions_label` named the array to transfer, and pymskt's readers then opened `get_scalar("labels")` regardless, so no value but the default had ever worked. Deleted in v0.4.0. `orig_cart` is also never read, but it is a slot in a fixed-layout list rather than a parameter, so it stays (`SCOPE.md` §2.5).* *§8.0.N found the variant a **fix** creates: ungating 58 `logger` calls from `if verbose:` left two `verbose` parameters with no reader at all (`SDFSamples.load_mesh_step`, `_process_meshes_for_wandb`), and they were exactly the two `_verbose_deprecation` had exempted from the bridge **because** their `verbose` was required. Both deleted with their last reader. Removing a gate is a move that can manufacture this class, so the sweep has to run after the cleanup, not before.* *§8.0.R swept the whole documented function surface for it and came back **empty** — every candidate was a polymorphic interface whose *sibling* reads the parameter (`epoch` on three decoders, `cache_path` on the two `_upgrade_cached_layout` implementations), which is the discriminator this class needs before #20's delete-the-parameter remedy is safe: judge a hook across every implementation, not per function. The live instances are all one frame up, at the **config layer**, where a translator between config keys and constructor arguments drops a key its siblings read — `_get_two_stage_params` dropped four (`KNOWN_ISSUES.md` § History 30) and `_get_implicit_params` drops `activation` (`SCOPE.md` §2.6).* |
| **Silent in-place mutation of caller data** | 7 | `get_pts_center_and_scale` mutates the passed array; all three in-repo callers pass `np.copy()` defensively, so the convention exists only as a habit at the call sites. *§8.0.N′ documented rather than changed the variant that depends on the argument's type: `cartilage_func._as_mesh` returns a `BoneMesh` argument unchanged, so it comes back carrying `labels` and `thickness (mm)`, and copies anything else. Nothing in NSM passes the former — `create_mesh` and `read_meshes` both produce the plain `Mesh` — so making it uniform is a behaviour change with no reported instance.* *§8.0.N closed [#55](https://github.com/gattia/nsm/issues/55), whose three sites needed three different answers and that is the finding: the class does not have one remedy. `compute_recon_loss`'s float32 downcast was **deleted** — its stated reason ('make sure the points are the same types') is false, pymskt's `pcu_sdf` casts both sides to float64 itself, so the code went with the rationale rather than being performed on a copy. The `sdf_gt` preprocess **copies**, because the clamp is its product. `interpolate_mesh` is **documented**, because carrying a mesh along the level set is the operation and a copy per call is not free — and the documentation existed already, at the private engine rather than at either public entry point, which is the failure worth remembering: a contract written in the frame the author had open.* |
