...
def test_deprecated_keywords_warn_and_unknown_ones_are_ignored(sphere_paths, caplog, capsys):
    """
    The deprecated flags are always on and ``mean`` never did anything. Both readers take
...
class TestSingleMeshReader:
    def test_its_outputs_on_each_path(self, sphere_paths):
        """
        A missing path is ``None``. Every path returns its points under ``pts``: the
        legacy ``xyz`` key was removed before any release carried it (#15).
        """
    ...
    def test_registration_takes_the_references_position_and_size(self, sphere_paths):
        """Similarity registration: the subject's size does not survive it."""
...
class TestMultiMeshReader:
    ...
    def test_a_supplied_icp_transform_is_used_instead_of_registering(self, sphere_paths):
        ...
        later pass, so all of a subject's points share one registration.
        """
        bone_path, cart_path = sphere_paths
        kwargs = dict(
            sigma=[0.1, 0.1],
    ...
    def test_a_none_sigma_draws_from_one_cube_around_all_surfaces(self, sphere_paths):
        """Bone-labelled samples can land in the cartilage's half of the joint cube."""
        result = read_meshes_get_sampled_pts(
            list(sphere_paths),
            sigma=[None, None],
            n_pts=[200, 200],
    ...
    def test_include_surf_in_pts_appends_each_surfaces_own_vertices(self, sphere_paths, sigma):
        """
        #17. On the uniform-cube path this configuration raised ``ValueError`` before the
        fix, and on the near-surface path it appended the wrong surface's vertices.
        """
        result = read_meshes_get_sampled_pts(
            list(sphere_paths),
            sigma=[sigma, sigma],
